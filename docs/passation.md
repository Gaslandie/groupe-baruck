...
Assistant guidé (2026-09-06, « chatbox faite maison ») : lanceur fixe en bas à droite sur toutes les pages montées par `PageShell` (pas la 404 ni Projets & Réalisations, qui passent par `ConstructionPage`). Composant client `src/components/assistant/SiteAssistant.tsx`, arbre de conversation typé dans `src/data/assistant.ts` (31 nœuds : 6 sujets — Studio photo, Hôtesses, JECA, Espoir de Vie, Le Groupe, Nous joindre — puis questions et réponses). **Aucune réponse n'est rédigée à part** : tout vient de `studioFaq`, `studioPriceGroups`, `hostessOffers`, `jecaEditions`, `edvPillars`, `edvCountries`, `edvTimeline`, `activities`, `pageTeasers`, `footers`, `contacts`, `hqAddress`, `hqHours` ; modifier ces données met le bot à jour. Pas de champ libre, pas d'IA, pas de tiers, pas de don. Sujet de la page courante proposé en premier (`topicByPath`). Pied du panneau : « Recommencer » et « Parler à quelqu'un » (WhatsApp siège, fixe, e-mail, formulaire). Fil et état ouvert/fermé conservés dans `sessionStorage` (clé `baruck-assistant`, identifiants de nœuds et libellés seulement, rien n'est écrit tant que le visiteur n'ouvre pas l'assistant) pour survivre aux liens internes proposés. Accessibilité : `role="dialog"` non modal, focus sur le titre à l'ouverture et retour au lanceur à la fermeture, Échap, fil `role="log" aria-live="polite"`, options = vrais boutons/liens. Animations `assistantIn` / `assistantDot` dans `globals.css`, neutralisées par `prefers-reduced-motion` (réponse immédiate, sans indicateur de frappe). Ancre `#carte` ajoutée à la section carte de Contact. Benchmark, choix et spécification : `docs/benchmark-chatbox-2026-09-06.md`. Test fonctionnel Chrome headless (DevTools Protocol) passé le 2026-09-06 : 30 contrôles (ouverture, options, prix, liens `basePath`, fil conservé après navigation, sortie humaine, Échap, restauration, reduced-motion).
