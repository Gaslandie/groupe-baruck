      ...
      <div className="grid auto-rows-[clamp(150px,15vw,225px)] grid-cols-4 gap-[.8rem] max-[1080px]:grid-cols-3 max-tablet:auto-rows-[150px] max-tablet:grid-cols-2 max-tablet:gap-2">
        {studioGallery.map((item) => (
          ...
          >
            ...
            />
            <figcaption className="absolute inset-x-0 bottom-0 bg-[linear-gradient(transparent,rgba(5,6,7,.82))] px-[1.1rem] pb-[.95rem] pt-[2.4rem] text-label uppercase tracking-[.14em] text-ivory">
              {item.caption}
            </figcaption>
          </figure>
        ))}
