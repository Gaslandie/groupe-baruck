    ...
    >
      ...
      <ol className="reveal-stagger m-0 grid list-none grid-cols-3 gap-[1.1rem] p-0 max-tablet:grid-cols-1">
        {hostessProcess.map((step) => (
          ...
          >
            <span aria-hidden="true" className="text-[.56rem] uppercase tracking-[.15em] text-accent">
              {step.number}
            </span>
            <h3 className="mb-[.9rem] mt-[1.4rem] font-display text-[clamp(1.35rem,1.8vw,1.8rem)] font-normal leading-[1.15]">
              {step.title}
            </h3>
            <p className="m-0 text-[.84rem] leading-[1.7] text-[#65645f]">{step.text}</p>
          </li>
        ))}
      </ol>
      ...
      <div className="reveal mt-[clamp(2.5rem,5vw,3.5rem)] border-t border-line pt-[1.4rem]">
        <h3 className="m-0 text-[.56rem] uppercase tracking-[.15em] text-accent">Types d’événements</h3>
        <ul className="m-0 mt-[1.1rem] flex list-none flex-wrap gap-[.5rem] p-0">
          {hostessTags.map((tag) => (
            <li
              key={tag}
              className="border border-line px-[.9rem] py-[.5rem] text-[.7rem] leading-none text-[#5f5d57]"
            >
              {tag}
            </li>
          ))}
