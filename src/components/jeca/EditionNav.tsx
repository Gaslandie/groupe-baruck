    ...
    >
      ...
      <div className="grid grid-cols-3 max-tablet:block">
        {jecaEditionNav.map((item) => (
          ...
          >
            <span className="row-[1/3] font-display text-lead font-bold italic">{item.number}</span>
            <strong className="font-display text-lead font-normal">{item.city}</strong>
            <small className="text-micro uppercase tracking-[.12em]">{item.dates}</small>
          </a>
        ))}
      </div>
    </nav>
