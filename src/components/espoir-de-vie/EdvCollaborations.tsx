    ...
    <section className="bg-edv-paper-deep px-[clamp(1.3rem,5vw,6rem)] py-[clamp(6rem,10vw,10rem)] max-tablet:px-[1.3rem] max-tablet:py-20">
      ...
      <div className="reveal-stagger grid grid-cols-3 border-t border-edv-line max-tablet:grid-cols-1">
        {edvCollaborations.map((collaboration) => (
          ...
          >
            <span className="text-[.55rem] font-extrabold tracking-[.15em] text-edv-ember">
              {collaboration.number}
            </span>
            <h3 className="mb-[.8rem] mt-16 font-display text-[clamp(1.65rem,2.5vw,2.8rem)] font-normal leading-none max-tablet:mt-8">
              {collaboration.title}
            </h3>
            <p className="m-0 max-w-[400px] text-[.8rem] leading-[1.7] text-edv-muted">{collaboration.text}</p>
          </article>
        ))}
      </div>
    </section>
