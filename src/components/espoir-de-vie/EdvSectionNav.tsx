  ...
  return (
    ...
    >
      {edvSectionNav.map((item) => (
        ...
        >
          <span className="row-span-2 font-display text-caption italic text-edv-ember transition-colors duration-[250ms] group-hover:text-edv-gold">
            {item.number}
          </span>
          <strong className="font-display text-display-sm font-normal">{item.title}</strong>
          <small className="mt-[.35rem] text-micro tracking-[.07em] text-edv-muted transition-colors duration-[250ms] group-hover:text-[rgba(255,255,255,.7)]">
            {item.subtitle}
          </small>
        </a>
      ))}
