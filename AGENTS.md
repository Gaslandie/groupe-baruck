...
- Arborescence : `src/app` (routes, `globals.css`, `fonts/`), `src/components/{layout,ui,home,about,services,jeca,espoir-de-vie,actualites,contact}`, `src/data` (contenus typés : site, media, home, services, jeca, espoir-de-vie, actualites, contact), `src/lib` (asset, actualites, boutique), `content/actualites/*.md` (articles, frontmatter décrit dans `docs/passation.md`), `content/boutique.json` (catalogue de la marque, chargé par `src/lib/boutique.ts`), `content/coordonnees.json` et `content/textes.json` (coordonnées et textes de l'accueil, importés via l'alias `@content/` résolu dans `next.config.ts`), `public/images/<domaine>/`.
...
- Tailles de texte : uniquement les rôles de l'échelle typographique de `globals.css` (`text-micro`, `text-label`, `text-caption`, `text-small`, `text-body`, `text-lead`, `text-title`, `text-title-lg`, `text-display-xs` à `text-display-xl`) ; jamais `text-[…rem]` ni `text-[clamp(…)]`, sauf les glyphes décoratifs (×, ‹, ›). Polices : `font-display` (Montserrat, variable, auto-hébergée dans `src/app/fonts/`) pour les titres et valeurs mises en avant, `font-sans` (Lato, quatre fichiers statiques 400/700 × normal/italique) pour tout le reste ; les deux remplacent Fraunces et Inter depuis le 2026-09-16. Lato n'ayant que 400 et 700, n'écrire que ces deux graisses dans du nouveau code : 500, 600 et 800 sont rabattues par le navigateur. Contrôle : `grep -rnE 'text-\[[0-9.]+rem\]|text-\[clamp' src` ne doit lister que ces glyphes. Les motifs répétés (`eyebrow`, `button`, `reveal`, `section-head`, marquee) vivent dans `@layer components` de `globals.css` ou dans un composant, jamais recopiés page par page. `@keyframes` dans `globals.css`.
...
- **Aucune décoration circulaire** (règle permanente de Mohamed, tous projets) : pas de cercle, anneau, arc, cercles concentriques ni halo en couronne posés en fond de section. Les ronds fonctionnels restent autorisés : puce de frise, avatar, bouton rond de l'assistant, badge, point d'état. Contrôle : `grep -rn "rounded-full" src --include=*.tsx | grep -i "border\|shadow"` ne doit lister que des ronds fonctionnels.
- **Icônes : SVG dessiné, jamais un caractère** (règle permanente) : les flèches et la croix de fermeture passent par `src/components/ui/Icon.tsx` (`arrow-up-right`, `arrow-up-left`, `arrow-up`, `arrow-down`, `arrow-left`, `arrow-right`, `close`). **Sens imposé** : `arrow-up-right` quand on quitte la page (autre page, lien externe), `arrow-down` pour une ancre `#…` de la page courante, `arrow-up-left` pour un retour (accueil, liste), `arrow-up` pour remonter en haut, `arrow-left`/`arrow-right` seulement pour un carrousel ou l'article voisin. La diagonale `arrow-down-right` a été supprimée le 2026-09-18. Les caractères Unicode d'interface (↗ ← → ↓ ×) ont une variante emoji : iOS les affiche en couleur et décalés. Garder les `<span>` autour de l'icône (les transitions `.button > span` et `.text-link span` s'y accrochent), `aria-hidden` sur l'icône et libellé accessible sur le lien ou le bouton. Contrôle : `grep -rlo '[↗↘↙↖↑←→]' out --include=*.html` doit être vide.
